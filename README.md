...
```
3. Put all the fastq files on the `./fastq/` folder or convert them from bam to fastq. See and adapt `makefastq.sh`. If you have bam files already you can skip step 4 and create links in the bam folder. Check `makebamopt2.sh` script to modify bam files to add "chr" prefix if needed. 
4. Alignment of all the fastq reads with the reference using BWA-MEM2 using Slurm jobs. If you have already bams you can skip this step. 
```
